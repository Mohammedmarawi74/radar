...
const FeedCard: React.FC<FeedCardProps> = ({ item, onAction }) => {
    ...
    const renderSignalAlert = () => {
        ...
        return (
            <div className={`mt-4 relative overflow-hidden rounded-2xl p-6 border ${isPos ? 'bg-emerald-950/5 border-emerald-100' : 'bg-rose-950/5 border-rose-100'}`}>
                {/* Background Decor */}
                <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-3xl opacity-20 ${isPos ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>

                <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-right">
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border ${isPos ? 'bg-white text-emerald-600 border-emerald-100' : 'bg-white text-rose-600 border-rose-100'}`}>
                        {isPos ? <TrendingUp size={32} /> : <AlertTriangle size={32} />}
                    </div>
                    <div className="flex-1">
                        <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-2 mb-2">
                            <span className="text-xs font-black tracking-widest uppercase text-gray-400">تحليل فني</span>
                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${isPos ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                                {isPos ? 'إشارة إيجابية' : 'تنبيه مخاطر'}
                            </span>
                        </div>
                        <div className="text-4xl font-black mb-2 tracking-tighter text-gray-900">
    ...
    const renderFact = () => (
        <div className="mt-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-8 border border-amber-100/50 relative overflow-hidden text-center group">
            <Quote size={80} className="absolute top-4 left-4 text-amber-500/10 rotate-180 transform transition-transform group-hover:scale-110" />

            <div className="relative z-10">
                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-amber-100 text-amber-500">
                    <Lightbulb size={24} className="fill-current" />
                </div>
                <h4 className="text-amber-900/60 font-bold text-xs uppercase tracking-[0.2em] mb-3">حقيقة استثمارية</h4>
                <p className="text-xl lg:text-2xl font-bold text-gray-900 leading-relaxed font-serif">
                    "{item.payload.fact}"
                </p>
            </div>
        </div>
    ...
    const renderBreakingNews = () => (
        <div className="mt-4 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-3xl p-6 relative overflow-hidden animate-pulse-slow shadow-xl shadow-red-600/30">
            <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-3xl -mr-10 -mt-10"></div>
            <div className="relative z-10 flex items-start gap-5">
                <div className="bg-white/20 p-3.5 rounded-2xl animate-bounce backdrop-blur-sm border border-white/20">
                    <Megaphone size={28} className="text-white fill-white" />
                </div>
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="bg-white text-red-700 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">عاجل</span>
                        <div className="w-1 h-1 rounded-full bg-red-300"></div>
                        <span className="text-red-100 text-xs font-bold font-mono tracking-wider">{new Date().toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <h3 className="text-2xl lg:text-3xl font-black leading-tight mb-3 tracking-tight">
                        {item.payload.headline}
                    </h3>
                    <p className="text-red-50 text-sm lg:text-base font-medium leading-relaxed opacity-90">
    ...
    const renderEvent = () => (
        <div className="mt-4 flex flex-col md:flex-row bg-white border border-gray-200 rounded-2xl overflow-hidden group hover:border-blue-300 transition-colors">
            <div className="bg-blue-50 p-6 flex flex-col items-center justify-center text-center w-full md:w-32 shrink-0 border-b md:border-b-0 md:border-l border-blue-100">
                <span className="text-xs font-bold text-blue-600 uppercase mb-1">{item.payload.month}</span>
                <span className="text-4xl font-black text-blue-800 mb-1">{item.payload.day}</span>
                <span className="text-xs font-medium text-blue-600/70">{item.payload.time}</span>
            </div>
            <div className="p-5 flex-1 relative">
                <div className="absolute top-4 left-4">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-md border ${item.payload.isOnline ? 'bg-green-50 text-green-700 border-green-100' : 'bg-orange-50 text-orange-700 border-orange-100'}`}>
                        {item.payload.isOnline ? 'ONLINE' : 'IN-PERSON'}
                    </span>
                </div>
                <h3 className="font-bold text-lg text-gray-900 mb-2 mt-4 md:mt-0">{item.payload.eventName}</h3>
    ...
    const renderDashboardContent = () => (
        <div className="group cursor-pointer mt-4 relative rounded-2xl overflow-hidden border border-gray-200 hover:border-blue-400 hover:shadow-2xl hover:shadow-blue-900/20 transition-all duration-500" onClick={() => navigate(`/dashboards?id=${item.payload.dashboardId}`)}>
            <div className="aspect-[16/9] w-full bg-slate-100 relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                    <span className="px-3 py-1 bg-white/90 backdrop-blur text-xs font-bold rounded-lg shadow-sm text-slate-800 flex items-center gap-1.5">
                        <Layers size={12} className="text-blue-600" /> لوحة تفاعلية
                    </span>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 text-white translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                    <div className="flex items-end justify-between">
                        <div>
                            <p className="text-blue-300 text-xs font-bold uppercase tracking-wider mb-1">بيانات حية</p>
                        ...
                        <div className="w-10 h-10 rounded-full bg-white text-blue-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
    ...
    const renderArticleContent = () => (
        <div className="mt-4 group cursor-pointer" onClick={() => navigate('/dataset/d1')}>
            {item.payload.imageUrl && (
                <div className="rounded-2xl overflow-hidden mb-4 relative aspect-[2/1] lg:aspect-[2.5/1]">
                    <img
            ...
            <div>
                <h3 className="text-xl lg:text-2xl font-bold text-gray-900 mb-2 group-hover:text-blue-700 transition-colors leading-tight">
                    {item.title}
                </h3>
                <p className="text-gray-500 text-sm lg:text-base leading-relaxed line-clamp-3">
                    {item.payload.excerpt}
                </p>
                <div className="mt-3 flex items-center text-blue-600 text-xs font-bold gap-1 group-hover:gap-2 transition-all">
                    اقرأ المزيد <ArrowDownRight size={14} />
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-gray-100 shadow-[0_2px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-300 mb-4 md:mb-6 group hover:-translate-y-1">
